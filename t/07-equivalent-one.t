...
subtest "Placeholders" => sub {
  ...
  eq_or_diff(
    $t1->_equiv_one( $phbaz, 1, \%t1 ),
    $t1->_equiv_placeholder( $phbaz, 1, \%t2 ),
...
subtest "Simple sublevel placeholders" => sub {
  my %ph;
  my $t1 = T1->new( val => 'foo' );
  my $ret = $t1->_equiv_one( T1->new( val => [ ph(val1), ph(val2) ] ), T1->new( val => [ 6, 12 ] ), \%ph );
...
subtest "Complex sublevel placeholders" => sub {
  my %ph;
  my $t1 = T1->new( val => 'foo' );
